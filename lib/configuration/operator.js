...
exports.defaultOperator = function (config) {
  config.registerOperator('or', ({ field, value }) => {
    const segOr = /\|/g;
    const segAnd = /\s+|\+|,/g;
    ...
    return { sql: sqls.length ? `(${sqls.join(` ${join} `)})` : '', arg };
  });
  config.registerOperator('or in', ({ field, value }) => {
    const _value = Array.isArray(value) ? value : value.split(',');
    let arg = [];
    const sqls = field.split(',').map(item => {
      arg.push(item, _value);
      return `(?? in (?))`;
    });

    return { sql: `(${sqls.join(` or `)})`, arg };
  });
  config.registerOperator('>', ({ field, value }) => {
  ...
  config.registerOperator('not in', ({ field, value }) => {
    return { sql: '?? not in (?)', arg: [field, value] };
  });
  config.registerOperator('date', ({ field, value }) => {
    const fields = field.split(',');
    // [value,format,symbol]
    if (fields.length === 2) return { sql: `DATE_FORMAT(??,"${fields[1]}") = ?`, arg: [fields[0], value] };
    if (fields.length === 3) return { sql: `DATE_FORMAT(??,"${fields[1]}") ${fields[2]} ?`, arg: [fields[0], value] };
    return { sql: 'DATE_FORMAT(??,"%Y-%m-%d") = ?', arg: [field, value] };
  });
  config.registerOperator('datetime', ({ field, value }) => {
    return { sql: 'DATE_FORMAT(??,"%Y-%m-%d %H:%i:%s") = ?', arg: [field, value] };
  });
